...
BATCH_SIZE = 10   # 한 번에 처리할 공고 수 (기존 5 → 10)
...
if __name__ == "__main__":
    ...
    if not pending_tasks:
        ...
        # 결과 수집 (row_num → 업데이트 데이터)
        results = {}  # row_num: (ai_data or None, error or None)
        ...
        for row_num, (ai_data, error) in results.items():
            if ai_data:
                ...
                # 오류 셀만 단독 업데이트 (배치에 포함하기 어려운 단일 셀)
                sheet.update_cell(row_num, 7, error)

        if all_cells_to_update:
            sheet.update_cells(all_cells_to_update)  # 전체 한 번에 업데이트
